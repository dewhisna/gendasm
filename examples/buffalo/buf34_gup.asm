* BUFFALO (Version 1.0 for GuP)
* "Bit User's Fast Friendly Aid to Logical Operation"
*
* Rev 1.0 - 9/26/08 - following changes made to convert from 68HC11 to GuP
*                   - modifications by Zach Peine
*                   - RAM location changed from 0000 to 0040
*                   - Registers location changed from 1000 to 0000
*                   - Buffalo memory space set to start at 0040
*                   - Added code to INIT routine for initializing the display
*                   - Removed COP code (there isn't currently a COP) from INPUT 
*                     method and added a check of the keyboard adapter
*                   - Added code to OUTPUT to write characters to the display
*                   - Changed ONSCI to use the new CPB register ($35) to set
*                     the baud rate. This allows easily setting the baud to
*                     anything between 300 and 115200.
*                   - Changed OUTCRLF to output to the display and wait until 
*                     the VGA adapter clears the rest of the line in memory
*                   - Removed EEPROM programming code. (There isn't one
*                     programmable by the CPU.)
*
* Origianl BUFFALO code for version 3.4 below
*
...
OUTPUT   EQU  *
	...
	psha
	cmpa #$08 	;backspace
	beq outputx1	;jump if backspace
	cmpa #$0d	;newline
	bne outputx2	;jump if not newline
	jsr outcrlfv2	;newline
	pula
	rts
outputx1 ; backspace
;	pshb
...
;         STAA BAUD        ;baud register
	pshx
	ldx #1311 ; 25.175 MHz / 9600 baud / 2 = 1311
	stx $35   ; new CPB register
	pulx
         LDAA #$00
         STAA SCCR1
         LDAA #$0C
...
;	pulb
	ldaa #$0d
	staa $37
	brset $38, $01, * ; wait until line is cleared
	rts
...
;         LDAA #$0D      ;cr
;	ldaa #$0d
         JSR  OUTPUT_v1    ;output a
;         LDAA #$00
;         JSR  OUTPUT_v1    ;output padding
;         LDAA #$0D
