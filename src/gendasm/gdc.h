...
protected:
	...
	virtual bool WriteDisassembly(std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);	// Writes the disassembly to the output file.  Default iterates through memory and calls correct functions.  Override as needed.
	virtual bool WritePostDisassembly(std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);	// Writes any info needed in the disassembly file after the main disassembly section, such as ".end".  Default is do nothing.  Override as needed

	virtual bool WritePreSection(std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);	// Writes any info needed in the disassembly file prior to a new section.  That is, a section that is loaded and analyzed.  A part of memory containing "not_loaded" areas would cause a new section to be generated.  This should be things like "org", etc.
	virtual bool WriteSection(std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);		// Writes the current section to the disassembly file.  Override as needed.
	virtual bool WritePostSection(std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);	// Writes any info needed in the disassembly file after a new section.  Default is do nothing, override as needed.

	virtual bool WritePreDataSection(std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);	// Writes any info needed prior to a data section -- that is a section that contains only DMEM_DATA directives.  This would be things like "DSEG", etc.  Default is do nothing. Override as needed.
	virtual bool WriteDataSection(std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);		// Writes a data section.
	virtual bool WritePostDataSection(std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);	// Writes any info needed after a data section.  Default is do nothing.  Typically this would be something like "ends"

	virtual bool WritePreCodeSection(std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);	// Writes any info needed prior to a code section -- such as "CSEG", etc.  Default is do nothing.  Override as needed.
	virtual bool WriteCodeSection(std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);		// Writes a code section.
	virtual bool WritePostCodeSection(std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);	// Writes any info needed after a code section.  Default is do nothing.  Typically this would be something like "ends"

	virtual bool WritePreFunction(std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);		// Writes any info needed prior to a new function -- such as "proc".  The default simply writes a separator line "===".  Override as needed
	virtual bool WriteIntraFunctionSep(std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);	// Writes a function's intra code separation.  The default simply writes a separator line "---".  Override as needed
	virtual bool WritePostFunction(std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);	// Writes any info needed after a function -- such as "endp".  The default simply writes a separator line "===".  Override as needed

	virtual bool ValidateLabelName(const TLabel & aName);		// Parses a specified label to make sure it meets label rules

	virtual bool ResolveIndirect(TAddress nAddress, TAddress& nResAddress, REFERENCE_TYPE nType) = 0;	// Pure Virtual that Resolves the indirect address specified by nAddress of type nType where nType is the reference type (i.e. Code locations vs. Data locations).  It returns the resolved address in nResAddress and T/F for mem load status
