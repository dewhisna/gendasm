...
protected:
	...
	virtual bool WriteSection(MEMORY_TYPE nMemoryType, std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);		// Writes the current section to the disassembly file.  Override as needed.
	virtual bool WritePostSection(MEMORY_TYPE nMemoryType, std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);	// Writes any info needed in the disassembly file after a new section.  Default is do nothing, override as needed.
	...
	virtual bool WriteDataSection(MEMORY_TYPE nMemoryType, std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);		// Writes a data section.
	virtual bool WritePostDataSection(MEMORY_TYPE nMemoryType, std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);	// Writes any info needed after a data section.  Default is do nothing.  Typically this would be something like "ends"

	virtual bool WritePreCodeSection(MEMORY_TYPE nMemoryType, std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);		// Writes any info needed prior to a code section -- such as "CSEG", etc.  Default is do nothing.  Override as needed.
	virtual bool WriteCodeSection(MEMORY_TYPE nMemoryType, std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);		// Writes a code section.
	virtual bool WritePostCodeSection(MEMORY_TYPE nMemoryType, std::ostream& outFile, std::ostream *msgFile = nullptr, std::ostream *errFile = nullptr);	// Writes any info needed after a code section.  Default is do nothing.  Typically this would be something like "ends"
