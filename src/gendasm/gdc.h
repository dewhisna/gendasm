...
public:
	...
	virtual bool AddLabel(MEMORY_TYPE nMemoryType, TAddress nAddress, bool bAddRef = false, TAddress nRefAddress = 0, const TLabel & strLabel = TLabel(), bool bMakeLabelPrimary = false);		// Sets strLabel string as the label for nAddress.  If nAddress is already set with that string, returns FALSE else returns TRUE or all OK.  If address has a label and strLabel = empty or zero length, nothing is added!  If strLabel is empty or zero length, an empty string is added to later get resolved to Lxxxx form.  If bAddRef, then nRefAddress is added to the reference list.  If bMakeLabelPrimary and strLabel isn't empty, then the label is put at the top of the list as the primary label
